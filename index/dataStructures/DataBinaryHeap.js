export default class DataHeap {
  ...
  header() {
    ...
    console.log(
      "  REMEMBER!!!! ",
      "Left child: 1*2  ;   Right chile: 1*2 + 1  ; find Parent: i/2"
    );
  ...
  maxHeap() { 
    let heap = this.values;  // Scoped to this function-wide

    this.print = () => heap;

    this.insert = function (elem) {
      heap.push(elem); // this will bubble only if 2 pre-existing

      if (heap.length > 2) {    // if needing to bubble
      this.bubbleUp();
      }
    };
