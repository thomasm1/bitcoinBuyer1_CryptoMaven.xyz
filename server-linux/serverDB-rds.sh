...
# Check if internet gateway is set, If it isn't there then do these:
...
# Create subnets for the DB & Web Servers in Multiple AZ1  
...
# Add rule that allows inbound SSH, HTTP, HTTP traffic ( from any source )
...
# Create subnet for the DB instances in RDS
aws rds create-db-subnet-group \
        --db-subnet-group-name "mysqlDBSubnet" \
        --db-subnet-group-description "Subnet group for Armchair-Bitcoinist-Parameter-Group databases instances" \
        --subnet-ids "$USEast1b_DbSubnetID" "$USEast1c_DbSubnetID"

# Create Security Group for Database RDS - MySQL from Web Security Group
...
# Add rule that allows inbound MySQL ( from any source )
...
### Create DB parameter group to monitor CRUD
aws rds create-db-parameter-group \
    --db-parameter-group-name armchairParamGroup \
    --db-parameter-group-family MySQL5.6 \
    --description "Armchair parameter group"

aws rds modify-db-parameter-group --db-parameter-group-name armchairParamGroup --parameters "ParameterName=general_log, ParameterValue=ON, Description=logParameter,ApplyMethod=immediate"
...
aws rds modify-db-instance --db-instance-identifier "$rdsInstID" --db-parameter-group-name armchairParamGroup
...
# aws rds delete-db-parameter-group --db-parameter-group-name "armchairParamGroup"